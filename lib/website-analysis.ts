...
async function getAiAnalysis(input: {
  ...
  const prompt = `You are AudiFox's senior technical SEO and web performance strategist.
...
// Shared implementation. Public route handlers must authorize before calling.
export async function analyzeAuthenticatedWebsite(request: Request) {
  try {
    ...
    for (let i = 0; i < 6; i += 1) {
      response = await fetch(url.href, {
        ...
        headers: { "User-Agent": "AudiFoxAudit/2.0", Accept: "text/html" },
      });
    ...
    const checks: AuditCheck[] = [
      ...
      { name: "Page title length", pass: !title || (title.length >= 30 && title.length <= 60), detail: title ? `${title.length} characters: ${title}` : "Evaluated after a title is added", fix: "Keep the title between 30 and 60 characters while preserving the page's primary topic and intent.", category: "Content" },
      ...
      { name: "Meta description length", pass: !description || (description.length >= 120 && description.length <= 160), detail: description ? `${description.length} characters` : "Evaluated after a description is added", fix: "Keep the meta description between 120 and 160 characters and include the page's specific value proposition.", category: "Content" },
